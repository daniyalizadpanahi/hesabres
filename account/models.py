...
class CustomUserManager(BaseUserManager):
    ...
    def create_superuser(self, national_id, phone_number, password=None, **extra_fields):
...
class Account(models.Model):
    user = models.ForeignKey(CustomUser, on_delete=models.DO_NOTHING, related_name='acc_user')
    signature = models.ForeignKey(CustomUser, on_delete=models.DO_NOTHING, related_name='acc_signature') # حق امضا
...
class Inventory(models.Model):
    class InventoryType(models.TextChoices):
        MOSQUE_DONATION = 'EMPLOYEE_DONATION', 'Donation for Mosque' # کمک به مسجد
...
class Transaction(models.Model):
    ...
    account = models.ForeignKey(Account, on_delete=models.DO_NOTHING, null=True, blank=True, related_name='trans_account')
...
class Loan(models.Model):
    borrower = models.ForeignKey(Account, on_delete=models.DO_NOTHING, related_name='loan_borrower')  # حساب وام‌گیرنده
    guarantor = models.ForeignKey(Account, on_delete=models.DO_NOTHING, null=True, blank=True, related_name='loan_guarantor')  # حساب ضامن وام
    ...
    blocked_from_guarantor = models.BooleanField(default=False)  # آیا این مبلغ از حساب ضامن مسدود شده یا نه
    ...
    def remaining_balance(self):
        return (self.amount // self.total_months) * (self.total_months - self.paid_months)  # محاسبه مانده بدهی
